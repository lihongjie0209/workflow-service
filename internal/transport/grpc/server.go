...
import (
	...
	apphealth "github.com/lihongjie0209/workflow-service/internal/health"
...
func NewServer(lc fx.Lifecycle, cfg config.Config, authService *auth.Service, authorizer platformauthz.Authorizer, workflowService *workflow.Service, healthService *apphealth.Service, idempotencyManager *idempotency.Manager, metrics *observability.Metrics, logger *slog.Logger) (*Server, error) {
	...
	if workflowService != nil {
		workflowv1.RegisterWorkflowServiceServer(grpcServer, newWorkflowServer(workflowService))
	}
...
func authenticateGRPC(ctx context.Context, method string, service *auth.Service, cfg config.Auth) (context.Context, error) {
	...
	if cfg.PSK.Enabled && auth.MatchesAny(method, cfg.PSK.GRPCMethods) {
		...
		return platformprincipal.WithContext(ctx, platformprincipal.Principal{ID: "workflow-service:psk", Type: platformprincipal.TypeServiceAccount}), nil
	}
