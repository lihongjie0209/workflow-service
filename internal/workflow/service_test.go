...
func TestServiceStartInstancePinsPublishedRevisionAndIdempotency(t *testing.T) {
	...
	got, err := service.StartInstance(actorContext("starter-1"), StartInstanceInput{
		TenantID: "tenant-1", DefinitionKey: "leave.approval", BusinessKey: "leave-42", Title: "Leave",
...
func TestServiceListMyTasksIgnoresCallerRoleClaims(t *testing.T) {
	...
	_, err := service.ListMyTasks(actorContext("user-1"), TaskFilter{
		TenantID: "tenant-1", RoleIDs: []string{"forged-admin"}, IncludeUnclaimed: false,
	})
	if err != nil {
...
func TestServiceTaskAuthorizationUsesResolvedRole(t *testing.T) {
	...
	store.claimTask = func(_ context.Context, _, _ string, _ int64, actor string) (Task, error) {
		if actor != "user-1" {
	...
	if _, err := service.ClaimTask(actorContext("user-1"), "tenant-1", "task-1", 2); err != nil {
		t.Fatalf("ClaimTask() error = %v", err)
	}

	service = mustService(t, store, &fakeAssignments{roles: []string{"viewer"}})
	if _, err := service.ClaimTask(actorContext("user-1"), "tenant-1", "task-1", 2); !errors.Is(err, ErrForbidden) {
...
func TestServiceRejectsStaleTaskActionBeforeRepositoryWrite(t *testing.T) {
	...
	store.claimTask = func(context.Context, string, string, int64, string) (Task, error) {
		called = true
		return Task{}, nil
	}
	service := mustService(t, store, &fakeAssignments{})
	_, err := service.ClaimTask(actorContext("user-1"), "tenant-1", "task-1", 2)
...
func mustService(t *testing.T, store Store, assignments AssignmentResolver) *Service {
	t.Helper()
	service, err := NewService(store, assignments)
	if err != nil {
...
type fakeStore struct {
	...
	claimTask           func(context.Context, string, string, int64, string) (Task, error)
}
...
func (f *fakeStore) PublishDefinition(context.Context, string, string, int64, string) (Definition, error) {
	return f.definition, nil
}
func (f *fakeStore) DisableDefinition(context.Context, string, string, int64, string) (Definition, error) {
	return f.definition, nil
}
func (f *fakeStore) GetDefinition(context.Context, string, string, uint32) (Definition, error) {
	return f.definition, nil
}
func (f *fakeStore) GetPublishedDefinitionByKey(context.Context, string, string) (Definition, error) {
...
func (f *fakeStore) GetInstance(context.Context, string, string) (Instance, error) {
	return f.instance, nil
}
func (f *fakeStore) CancelInstance(context.Context, string, string, int64, string, string) (Instance, error) {
	return f.instance, nil
}
func (f *fakeStore) ListInstances(context.Context, InstanceFilter) (Page[Instance], error) {
	return Page[Instance]{}, nil
}
func (f *fakeStore) GetTask(context.Context, string, string) (Task, error) { return f.task, nil }
...
func (f *fakeStore) ClaimTask(ctx context.Context, tenantID, id string, version int64, actor string) (Task, error) {
	if f.claimTask != nil {
		return f.claimTask(ctx, tenantID, id, version, actor)
	}
	return f.task, nil
}
