...
CREATE TABLE workflow_instances (
  ...
  status TEXT NOT NULL CHECK (status IN ('running','completed','rejected','cancelled','failed')),
  ...
  UNIQUE (tenant_id,definition_id,business_key)
);
...
CREATE TABLE workflow_tasks (
  ...
  name TEXT NOT NULL,
  assignee_type TEXT NOT NULL CHECK (assignee_type IN ('user','role','starter','expression')),
  assignee TEXT NOT NULL,
  claimed_by TEXT NOT NULL DEFAULT '',
  status TEXT NOT NULL CHECK (status IN ('pending','claimed','approved','rejected','cancelled','expired')),
